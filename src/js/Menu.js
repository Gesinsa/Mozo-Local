...
let Departa = () => {
	...
	Depart.forEach((A) => {
		...
		div.addEventListener("click", async () => {
			...
			localStorage.setItem("Departamento", JSON.stringify({ "Code": A.Code, "Name": A.Name }));
...
(async () => {
	...
	if (parseInt(Config.Area) == 1) {
		Retornar.textContent = "Volver a Grupo";
		Retornar.addEventListener("click", async () => {
			window.location.href = "MenuAreas";
		});
	} else {
